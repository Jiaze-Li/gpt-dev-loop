...
| D22 | Both AGY families run through a workspace-local `reviewloop-minimal` custom agent (`inheritCustomizations: false`), provisioned deterministically/idempotently into the scratch workspace only — never into the user's HOME or AGY global config. Provisioning failure fails closed (AGY families UNAVAILABLE); ReviewLoop never falls back to AGY's ambient default agent. Production transport is **live-certified** by a controlled single-turn smoke (`gemini-3.8-flash-high` Supervisor: input 150668→6713, cache-read 656768→8128, ~92.7s→8.4s); `agy:gemini` stays `highContext` / out of automatic routing pending a full controller-level E2E. |
| D23 | `usageVolume` (the `MAX_USAGE_VOLUME` safety ceiling) is provider/family-aware (`usageAccountingOf`), keyed off the family/provider in the CallIntent, never a model name. Precedence: authoritative provider-reported total → family fallback (`openai` = input+output, cache-read is a subset; `anthropic` = input+output+cache_creation+cache_read, separate categories) → conservative additive flagged `semanticsKnown:false` for AGY-without-total / unknown providers. Cached tokens are never double-counted; raw per-field breakdown and `usageAccounting` provenance are persisted on every spend record. UNKNOWN != ZERO preserved. |
